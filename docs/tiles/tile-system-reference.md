...
이 문서는 타일 작업에 필요한 프로젝트 구조와 확인 절차를 정리한다. 수치와 관계는 현재 브랜치의 직렬화 파일을 기준으로 확인했으며, 실제 코드·씬·에셋이 항상 최종 기준이다.
...
### 1.7 실제 배치에서 확인된 점유 형태

Main 씬의 Wall Tilemap을 역참조한 결과는 다음과 같다.

- Wall_CreamWallPaper는 사각 방의 1셀 두께 외곽선을 이룬다.
- Close_Door는 좌표 (4, 3)과 (4, 4)에 세로로 인접한 두 셀이다.
- 따라서 벽을 항상 논리 2셀 두께로 배치한다는 규칙은 현재 Main 씬과 맞지 않는다.
- 문도 한 사례의 방향을 프로젝트 전체 규칙으로 일반화하지 않는다. 기준 씬과 규칙의 이웃 조건으로 점유 셀과 방향을 확인한다.
...
- 벽과 문의 두께, 방향, 점유 셀 수는 기준 씬에서 측정한다.
...
- 문과 가구의 전체 점유 영역이 벽과 통로를 침범하지 않는가?
